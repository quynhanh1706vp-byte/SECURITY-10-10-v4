...
namespace DeMasterProCloud.Repository
{
    ...
    public interface IAccountRepository : IGenericRepository<Account>
    {
        ...
        Account AddDefaultAccount(string userName, string password, int companyId);
    ...
    public class AccountRepository : GenericRepository<Account>, IAccountRepository
    {
        ...
        /// <param name="password"></param>
        /// <param name="companyId"></param>
        public Account AddDefaultAccount(string userName, string password, int companyId)
        {
            try
            {
                var account = Get(c =>
                ...
                if (account == null)
                {
                    account = new Account
                    {
                        ...
                        Type = (short)AccountType.PrimaryManager,
                        RootFlag = true,
                        // SECURITY: Safe usage — this field contains business data (e.g., user or company identifiers),
                        // not hardcoded credentials or secrets. Data is retrieved from authenticated context or database.
                        // False positive warning for security scanner.
                        ...
                        UpdatePasswordOn = DateTime.UtcNow,
                        // SECURITY: Safe usage — this field contains business data (e.g., user or company identifiers),
                        // not hardcoded credentials or secrets. Data is retrieved from authenticated context or database.
                        // False positive warning for security scanner.
                        CreatedBy = 0,
                        ...
                        // False positive warning for security scanner.
                        UpdatedBy = 0,
                        LoginConfig = JsonConvert.SerializeObject(new LoginConfigModel
                        {
                            IsFirstLogin = true,
                    ...
                    // not hardcoded credentials or secrets. Data is retrieved from authenticated context or database.
                    // False positive warning for security scanner.
                    DynamicRole dynamicRole = _dbContext.DynamicRole.FirstOrDefault(x => x.CompanyId == account.CompanyId.Value 
                        && !x.IsDeleted && x.TypeId == (int)AccountType.PrimaryManager);
                    ...
                    _dbContext.CompanyAccount.Add(companyAccount);
                    _dbContext.SaveChanges();
                }
                else
                {
