...
namespace DeMasterProCloud.Repository
{
    ...
    {
        public static void Initialize(IUnitOfWork unitOfWork, IConfiguration configuration, IServiceProvider serviceProvider = null)
        {
            //Database migrate
            ...
            if (!unitOfWork.AppDbContext.Company.Any())
            {
                ...
                // Create default company using CompanyService.Add (same as API)
                if (serviceProvider == null)
                {
                    throw new Exception("[DbInitializer] IServiceProvider is required to create default company with all default settings");
                }

                // Get ICompanyService dynamically to avoid circular dependency
                var companyServiceType = Type.GetType("DeMasterProCloud.Service.ICompanyService, DeMasterProCloud.Service");
                var companyService = serviceProvider.GetService(companyServiceType);

                if (companyService == null)
                {
                    throw new Exception("[DbInitializer] ICompanyService not found in service provider");
                }
                ...
                var companyModel = new CompanyModel
                {
                    ...
                    Code = null,
                    Remarks = "",
                    EventLogStorageDurationInDb = 0,
                    EventLogStorageDurationInFile = 0,
                    TimeLimitStoredImage = 0,
                    TimeLimitStoredVideo = 0,
                    LimitCountOfUser = 0,
                    EnableReCheckImageCamera = false,
                    TimeLimitCheckImageCamera = 0,
                    AutoSyncUserData = true
                };

                // Call Add method using reflection
                var addMethod = companyServiceType.GetMethod("Add", new[] { typeof(CompanyModel) });
                if (addMethod == null)
                {
                    throw new Exception("[DbInitializer] Add method not found on ICompanyService");
                }

                companyId = (int)addMethod.Invoke(companyService, new object[] { companyModel });
                unitOfWork.Save();
                Console.WriteLine("[DbInitializer] Created default company using CompanyService.Add (same as API)");

                // Add a system admin as a default account to default company.
                unitOfWork.AccountRepository.AddDefaultAccount(
                    configuration[Constants.Settings.DefaultAccountUsername],
                    configuration[Constants.Settings.DefaultAccountPassword], companyId);

                Console.WriteLine("[DbInitializer] Created default account to company");
            }
        }
    }
}
