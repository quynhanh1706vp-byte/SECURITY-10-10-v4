...
namespace DeMasterProCloud.Api.Controllers
{
    ...
    public class EventLogController : Controller
    {
        ...
        public IActionResult UploadVideoEventLog(string rid, string startTime, string hash, IFormFile file)
        {
            ...
            // Security: HMAC-SHA256 for cryptographically secure device authentication
            // Using company.SecretCode as the secret key ensures only authorized devices can upload videos
            if (string.IsNullOrEmpty(hash) || !CryptographyHelper.VerifyHmacSha256Hash(company.SecretCode, startTime + rid + company.Code, hash))
        ...
        public IActionResult UploadImageToEventLog(string rid, [FromBody] UploadImageEventModel model)
        {
            ...
            // Security: HMAC-SHA256 for cryptographically secure device authentication
            // Using company.SecretCode as the secret key ensures only authorized devices can upload images
            bool verifyHash = CryptographyHelper.VerifyHmacSha256Hash(company.SecretCode, $"{rid}{model.Id}", model.Hash);
            if (!verifyHash)
            {
