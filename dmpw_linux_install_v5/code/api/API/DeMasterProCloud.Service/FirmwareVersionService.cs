                        ...
                        if (!string.IsNullOrEmpty(firmwareType))
                        {
                            ...
                            if (!terminalAndroids.Contains(device.DeviceType))
                            {
                                ...
                                if (device.CompanyId.HasValue)
                                {
                                    ...
                                    string hash = CryptographyHelper.GetHmacH256($"{firmwareVersion.FileName}{companyOfDevice.Code}{textUtcNow}", companyOfDevice.SecretCode);
        ...
        public bool CheckHashFirmware(string fileName, string hash)
        {
            try
            {
                ...
                foreach (var company in companies)
                {
                    ...
                    {
                        // Security: HMAC-SHA256 for cryptographically secure firmware download verification
                        if (CryptographyHelper.VerifyHmacSha256Hash(company.SecretCode, $"{fileName}{company.Code}{itemUtcNow}", hash))
                        {
                            return true;
                        }
                    }
