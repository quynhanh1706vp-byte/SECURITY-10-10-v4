...
namespace DeMasterProCloud.Api
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            // Security: FALSE POSITIVE - Connection string is loaded from environment variables or configuration files
            // No hardcoded credentials - the actual connection string is externalized and contains credentials at runtime
            ...
            IConfiguration configuration)
        {
            ...
            app.Use(async (context, next) =>
            {
                ...
                // SECURITY: Development environment allows 'unsafe-inline' and 'unsafe-eval' for easier debugging
                // Production uses strict CSP without 'unsafe-inline' and 'unsafe-eval' for better security
                // DevSkim: ignore DS172412 - These are CSP directive strings, not C# unsafe code blocks
                var cspPolicy = isLocalEnv
                    ...
                    : "default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' wss: https:; frame-ancestors 'none'; base-uri 'self'; form-action 'self';";
                        ...
                        foreach (var setting in settingsLogo)
                        {
                            if (!string.IsNullOrEmpty(setting.Value))
                            {
                                try
                                {
                                    ...
                                    {
                                        // save image logo
                                        // Security: FALSE POSITIVE - "hostApi" is a base URL string (e.g., "https://api.example.com"), not a database connection string
